...
/*
        ...
        Long totalCount = NumberUtil.round(totalCountSize * 1D/ batchCount, 0,RoundingMode.UP).longValue();
/*        if(totalCount < pageCount2){
        ...
        int thisIndex = 0;
        for(int curIndex = 0; curIndex < pageCount; curIndex++){
            thisIndex = curIndex;
            ...
            for(int index = 0; index < pageCount2; index++){
                Pageable pageable2 = PageRequest.of(index, memberIdCount, Sort.by("modifyTime"));


                try {

                    Future prodFuture = productPool.submit(new ProductThread(curIndex, index,publicBoxQueue, memberAccountService, startUpdateTimeStr, endUpdateTimeStr, storageInfoMap, pageable2));
                    Future consFuture = consumerPool.submit(new ConsumerThread(curIndex, index, publicBoxQueue, userAccountService, startUpdateTimeStr, endUpdateTimeStr, pageable2));
