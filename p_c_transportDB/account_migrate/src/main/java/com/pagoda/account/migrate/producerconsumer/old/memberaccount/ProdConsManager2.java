...
/*
        ...
        final RejectedExecutionHandler handler2 = (r, executor) -> {
            log.info("太忙了,ConsumerThread "+ executor.toString());
            // msgQueue.offer(((DBThread) r).getMsg());
            if (!executor.isShutdown()) {
               // log.info("ConsumerThread Handler 尝试 重试 TaskNum " + ((ConsumerThread) r).getTaskNum());
                r.run();
            }
        };
       ...
       // Integer minMemberId = new BigInteger(minAndMaxAndCountMemberIdMap.get("minMemberId").toString()).intValue();
        ...
        for(int i = 0;  i < countNum; i++){
            ...
            try {
                ...
                Future consFuture = consumerPool.submit(new ConsumerThread(i, publicBoxQueue, userAccountService, startMemberId, endMemberId));
