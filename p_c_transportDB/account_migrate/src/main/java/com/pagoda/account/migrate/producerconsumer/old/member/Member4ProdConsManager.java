...
public class Member4ProdConsManager {
    ...
    public void  run() {
        ...
        BlockingQueue publicBoxQueue= new LinkedBlockingQueue(10);   //定义了一个大小为5的盒子
        //为多生产者和多消费者分别开创的线程池
        ThreadPoolExecutor productPool =
                new ThreadPoolExecutor(2, 10, 60, TimeUnit.MILLISECONDS, new ArrayBlockingQueue(2), handler1);//new ThreadPoolExecutor.CallerRunsPolicy()
        ThreadPoolExecutor consumerPool =
                new ThreadPoolExecutor(4, 10, 60, TimeUnit.MILLISECONDS, new ArrayBlockingQueue(3), handler2);
        ...
        //TODO
        String beginUpdateTimeStr = "20161101000000"; //2016-11-02 05:05:48  20161101000000
        String terminalUpdateTimeStr = "20161102000000";//"20160201000000" 20161101000000-20170101000000

        //TODO
        int batchCount = 5000 ;
        Long totalCount = 115002L;
//int terminalMemberId = 50000;
        ...
        for(int curIndex = 0; curIndex < pageSize; curIndex++){
            ...
            Pageable pageable = PageRequest.of(curIndex, batchCount);//Sort.by("modifyTime")
            log.info("【页码={}, startUpdateTime={}, endUpdateTime={}】", curIndex, startUpdateTimeStr, endUpdateTimeStr);

            try {

                Future prodFuture = productPool.submit(new ProductThread(curIndex, publicBoxQueue, memberService, startUpdateTimeStr, endUpdateTimeStr, pageable));
                Future consFuture = consumerPool.submit(new ConsumerThread(curIndex, publicBoxQueue, userService, startUpdateTimeStr, endUpdateTimeStr, pageable));
